    ...
    SuperOperators /*, ERC1155*/
{
    ...
    // ERC20SubToken[3] erc20s;
    // event SubToken(ERC20SubToken subToken);
    // constructor(address _to, uint256 supply0, uint256 supply1, uint256 supply2) public {
    //     deploySubToken(0, supply0, _to);
    //     deploySubToken(1, supply1, _to);
    //     deploySubToken(2, supply2, _to);
    // }
    // function deploySubToken(uint8 index, uint256 supply, address _to) internal {
    //     ERC20SubToken subToken = new ERC20SubToken(this, index);
    //     packedTokenBalance[_to] = packedTokenBalance[_to].updateTokenBalance(index, supply, ObjectLib64.Operations.REPLACE);
    //     totalSupplies[index] = supply;
    //     subToken.emitTransferEvent(address(0), _to, supply);
    //     erc20s[index] = subToken;
    ...
    //         _checkERC1155AndCallSafeTransfer(metaTransactionContracts[msg.sender] ? _from : msg.sender, _from, _to, _id, _value, "", true),
    //         "failCheck"
    //     );
    // }
    // function _transferFrom(address _from, address _to, uint256 _id, uint256 _value) internal {
    //     require(_to != address(0), "Invalid to address");
    //     ERC20SubToken erc20 = erc20s[_id];
    ...
    //     }
    //     packedTokenBalance[_from] = packedTokenBalance[_from].updateTokenBalance(_id, _value, ObjectLib64.Operations.SUB);
    ...
    //     _transferFrom(_from, _to, _id, _value);
    //     require( // solium-disable-line error-reason
    //         _checkERC1155AndCallSafeTransfer(metaTransactionContracts[msg.sender] ? _from : msg.sender, _from, _to, _id, _value, _data, false),
    //         "failCheck"
    //     );
    // }
    ...
    //     require(_checkERC1155AndCallSafeBatchTransfer(metaTransactionContracts[msg.sender] ? _from : msg.sender, _from, _to, _ids, _values, _data)); // solium-disable-line error-reason
    // }
    ...
    //     require(
    //         _from == msg.sender || _superOperators[msg.sender] || operatorsForAll[_from][msg.sender] || metaTransactionContracts[msg.sender],
    //         "not authorized"
    //     );
    //     uint256 balFrom = packedTokenBalance[_from];
    //     uint256 balTo = packedTokenBalance[_to];
    //     for (uint256 i = 0; i < _ids.length; i++) {
    //         ERC20SubToken erc20 = erc20s[_ids[i]];
    ...
    //     require(msg.sender == _sender || metaTransactionContracts[msg.sender] || _superOperators[msg.sender], "require meta approval");
