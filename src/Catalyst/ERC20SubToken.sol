    ...
    SuperOperators /*, ERC20 */
{
    ...
    constructor(ERC1155Core _core, uint8 _index) public {
    ...
    function transferFrom(address _from, address _to, uint256 _amount) external returns (bool success) {
        if (msg.sender != _from && !_superOperators[msg.sender] && !origin.core.isApprovedForAll(_from, msg.sender)) {
    ...
    function approveFor(address from, address _spender, uint256 _amount) external returns (bool success) {
    ...
    function emitTransferEvent(address _from, address _to, uint256 _amount) external {
