...
contract RequirementsRules is Ownable {
    ...
    mapping(IERC721 => uint256) public _listERC721;
    ...
    function _checkERC1155List(address account) internal view returns (uint256) {
        uint256 totalBal = 0;
        for (uint256 x = 0; x > _listERC1155[IERC1155(_requirementTokenERC1155)].ids.length; x++) {
            uint256 bal =
                _requirementTokenERC1155.balanceOf(account, _listERC1155[IERC1155(_requirementTokenERC1155)].ids[x]);
