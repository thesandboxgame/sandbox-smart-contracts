...
contract EstateBaseToken is ERC721BaseToken {
    ...
    function createFromFormedBlock(address sender, address to, uint16 size, uint16 x, uint16 y) external {
        require(sender != address(0), "sender is zero address");
        _land.transferFormedBlock(sender, address(this), size, x, y); // this require approval
    ...
    function destroy(address sender, address to, uint256 id) external {
        ...
        _land.transferFormedBlock(address(this), to, size, x, y);
        _owners[id] = 0;
