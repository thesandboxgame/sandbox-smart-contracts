...
contract LandBaseToken is ERC721Events, SuperOperators, MetaTransactionReceiver {
    ...
    function mintBlock(address to, uint16 size, uint16 x, uint16 y) external {
        ...
        if (size <= 12) {
            require(
                _owners[LAYER_12x12 + (x/12) * 12 + ((y/12) * 12) * GRID_SIZE] == address(0),
            ...
            for (uint16 x12i = x; x12i < toX; x12i += 12) {
                for (uint16 y12i = y; y12i < toY; y12i += 12) {
                    ...
                    require(_owners[id12x12] == address(0), "Already minted as 12x12");
                }
            }
        }
        ...
        if (size <= 6) {
            ...
            for (uint16 x6i = x; x6i < toX; x6i += 6) {
                for (uint16 y6i = y; y6i < toY; y6i += 6) {
                    ...
                    require(_owners[id6x6] == address(0), "Already minted as 6x6");
                }
            }
        }
        ...
        if (size <= 3) {
            ...
            for (uint16 x3i = x; x3i < toX; x3i += 3) {
                for (uint16 y3i = y; y3i < toY; y3i += 3) {
                    ...
                    require(_owners[id3x3] == address(0), "Already minted as 3x3");
                }
            }
        }
        ...
        for (uint16 xi = x; xi < x+size; xi++) {
            for (uint16 yi = y; yi < y+size; yi++) {
                ...
                require(_owners[id1x1] == address(0), "Already exists");
    ...
    function _ownerOf(uint256 id) internal view returns (address) {
        ...
        if (owner1x1 != address(0)) {
            ...
            address owner3x3 = _owners[LAYER_3x3 + x + y * GRID_SIZE];
            if (owner3x3 != address(0)) {
                return owner3x3;
            } else {
                address owner6x6 = _owners[LAYER_6x6 + x + y * GRID_SIZE];
                if (owner6x6 != address(0)) {
                    return owner6x6;
                } else {
                    address owner12x12 = _owners[LAYER_12x12 + x + y * GRID_SIZE];
                    if (owner12x12 != address(0)) {
                        return owner12x12;
                    } else {
                        return _owners[LAYER_24x24 + x + y * GRID_SIZE];
                    }
                }
            }
        }
    ...
    */
    ...
    // no safe version ? (as )this would require to call _checkOnERC721Received on all of them)
    // TODO add test for batch
    /**
     * @notice Transfer many token between 2 addresses
     * @param from The sender of the token
     * @param to The recipient of the token
    ...
    */
    ...
    {
        return _operatorsForAll[owner][operator] || _superOperators[operator]; // TODO add test for superOperators
    }
