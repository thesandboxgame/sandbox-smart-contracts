...
ERC998ERC20TopDown, ERC998ERC20TopDownEnumerable {
    ...
    function _transferFrom(address _from, address _to, uint256 _tokenId) private {
        require(_from != address(0), "_from not set");
        require(tokenIdToTokenOwner[_tokenId] == _from, "_from not the owner");
        require(_to != address(0), "_to not set");
        ...
        if(msg.sender != _from) {
            ...
            require(
                ...
                rootOwnerAndTokenIdToApprovedAddress[_from][_tokenId] == msg.sender, "Not aprroved");
        }
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId) external {
        ...
        if (isContract(_to)) {
            ...
            require(retval == ERC721_RECEIVED_OLD, "safeTransferFrom1 ERC721 does not match.");
        }
    }
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes _data) external {
        ...
        if (isContract(_to)) {
            ...
            require(retval == ERC721_RECEIVED_OLD, "safeTransferFrom2 ERC721 does not match.");
        }
    }
    ...
    function transferChild(uint256 _fromTokenId, address _to, address _childContract, uint256 _childTokenId) external {
        uint256 tokenId = childTokenOwner[_childContract][_childTokenId];
        require(tokenId > 0 || childTokenIndex[tokenId][_childContract][_childTokenId] > 0, "child doesn't exists");
        require(tokenId == _fromTokenId, "child equals parent");
        require(_to != address(0), "_to not set");
        address rootOwner = address(rootOwnerOf(tokenId));
        require(
            rootOwner == msg.sender || 
            tokenOwnerToOperators[rootOwner][msg.sender] ||
            rootOwnerAndTokenIdToApprovedAddress[rootOwner][tokenId] == msg.sender, "sender not approved");
        removeChild(tokenId, _childContract, _childTokenId);
    ...
    function onERC721Received(address, address _from, uint256 _childTokenId, bytes _data) external returns (bytes4) {
        require(_data.length > 0, "_data must contain the uint256 tokenId to transfer the child token to.");
        // convert up to 32 bytes of_data to uint256, owner nft tokenId passed as uint in bytes
        uint256 tokenId;
