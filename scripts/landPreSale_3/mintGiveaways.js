const fs = require("fs");
const parseSheet = require("../../lib/parseSheeteet");
...
async function mint(x, y, size, address) {
  ...
  if (!landToBuy) {
    console.error(`cannot find land ${size} : ${x - 204}, ${y - 204}`);
    // process.exit(1);
    return;
  }
...
(async () => {
  const landGroups = [];
  const giveaway = await parseSheet(
    "1ND3IZdBRaEdWDaoBIqMnp4tS4FIZEWomJ9c0n1Lmw04",
    "ROUND 3 — Bounties Giveaway (LANDS)",
    {
      startRow: 4,
      endRow: 33,
      fields: {
      ...
      filter: (object) => object.coordinates && object.name && object.address,
    }
  );
