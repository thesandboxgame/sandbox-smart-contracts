...
async function mintBatch({validate, url, creator, options, assetIds, checks}) {
  ...
  for (const assetId of assetIds) {
    ...
    if (assetData && assetData.asset) {
      ...
      if (assetMetadataData.metadata.sandbox.creator !== creatorWallet) {
        reportErrorAndExit(
          `creator wallet do not match, metadata (${assetMetadataData.metadata.sandbox.creator})  != wallet ${creatorWallet}`
        );
      }
  ...
  if (assetBatchInfo) {
    ...
    for (let i = 0; i < assetIds.length; i++) {
      ...
      if (!(propertiesTotalValue === expectedValue)) {
        reportErrorAndExit(
          `asset ${assetId} with rarity ${rarity} got wrong total value ${propertiesTotalValue} it should be  ${expectedValue}`
        );
      }
      if (!(maxValue <= expectedMaxValue)) {
        reportErrorAndExit(
          `asset ${assetId} with rarity ${rarity} got wrong max value ${maxValue} it should be  ${expectedMaxValue}`
        );
      }
    }
    ...
    if (checks) {
      if (checks.creatorWallet && checks.creatorWallet.toLowerCase() !== creatorWallet.toLowerCase()) {
        reportErrorAndExit(
          "creatorWallet not expected (expect " + checks.creatorWallet + " but got " + creatorWallet + ")"
        );
      }
    ...
    const isPackIdUsed = await call(
      ...
      creatorWallet,
      packId,
      suppliesArr.filter((n) => n > 1).length
    );
    if (isPackIdUsed) {
