...
describe("Permit", function () {
  ...
  it("ERC20 Approval event is emitted when msg.sender == owner", async function () {
    ...
    const receipt = await permitContract.permit(wallet.address, others[1], TEST_AMOUNT, deadline, sig.v, sig.r, sig.s).then((tx) => tx.wait());;
  ...
  it("Permit function reverts if deadline has passed", async function () {
    ...
    await expect(permitContract.permit(wallet.address, others[1], TEST_AMOUNT, deadline, sig.v, sig.r, sig.s)).to.be.revertedWith("PAST_DEADLINE");
  });
  ...
  it("Permit function reverts if owner is zeroAddress", async function () {
    ...
    await expect(permitContract.permit(zeroAddress, others[1], TEST_AMOUNT, deadline, sig.v, sig.r, sig.s)).to.be.revertedWith("INVALID_SIGNATURE"
    );
  });
  ...
  it("Permit function reverts if owner != msg.sender", async function () {
    ...
    await expect(permitContract.permit(others[2], others[1], TEST_AMOUNT, deadline, sig.v, sig.r, sig.s)).to.be.revertedWith("INVALID_SIGNATURE"
    );
  });
  ...
  it("Permit function reverts if spender is not the approved spender", async function () {
    ...
    await expect(permitContract.permit(wallet.address, others[2], TEST_AMOUNT, deadline, sig.v, sig.r, sig.s)).to.be.revertedWith("INVALID_SIGNATURE"
    );
  });
  ...
  it("Non-approved operators cannot transfer ERC20 until approved", async function () {
    ...
    // TODO: expect(receiverOriginalBalance).to.equal(0);
    ...
    await expect(sandContractAsSpender.transferFrom(wallet.address, others[2], TEST_AMOUNT)).to.be.revertedWith(
      "Not enough funds allowed"
    );
    await permitContract.permit(wallet.address, others[1], TEST_AMOUNT, deadline, sig.v, sig.r, sig.s).then((tx) => tx.wait());;
    ...
    // TODO: expect(receiverNewBalance).to.equal(TEST_AMOUNT.add(receiverOriginalBalance));
  });
  ...
  it("Approved operators cannot transfer more ERC20 than their allowance", async function () {
    ...
    const sandContractAsSpender = await sandContract.connect(sandContract.provider.getSigner(others[1]));
    await permitContract.permit(wallet.address, others[1], TEST_AMOUNT, deadline, sig.v, sig.r, sig.s).then((tx) => tx.wait());;
    await expect(sandContractAsSpender.transferFrom(wallet.address, others[2], TEST_AMOUNT.mul(2))).to.be.revertedWith(
      "Not enough funds allowed"
    );
  });
  ...
  it("Approved operators cannot transfer more ERC20 than there is", async function () {
    ...
    const sandContractAsSpender = await sandContract.connect(sandContract.provider.getSigner(others[1]));
    await permitContract.permit(wallet.address, others[1], TEST_AMOUNT, deadline, sig.v, sig.r, sig.s).then((tx) => tx.wait());;
    await expect(sandContractAsSpender.transferFrom(wallet.address, others[2], TEST_AMOUNT)).to.be.revertedWith("not enough fund");
  });
});
