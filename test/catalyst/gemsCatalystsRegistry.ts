...
describe('GemsCatalystsRegistry', function () {
  it('getMaxGems for catalystId = 1 should be 1', async function () {
    const { gemsCatalystsRegistry, commonCatalyst } = await setupGemsAndCatalysts();
  ...
  it('getMaxGems for non existing catalystId should fail', async function () {
    ...
    expect(gemsCatalystsRegistry.getMaxGems(10)).to.be.revertedWith("CATALYST_DOES_NOT_EXIST");
  });

  it('burnCatalyst should burn 2 common catalysts from deployer account', async function () {
    const { gemsCatalystsRegistry, commonCatalyst, accounts } = await setupGemsAndCatalysts();
    ...
    await gemsCatalystsRegistry.connect(ethers.provider.getSigner(deployer)).burnCatalyst(deployer, catalystId, burnAmount);
  ...
  it('burnGem should burn 3 power gems from deployer account', async function () {
    const { gemsCatalystsRegistry, powerGem, accounts } = await setupGemsAndCatalysts();
    const { deployer } = accounts;
    ...
    await gemsCatalystsRegistry.connect(ethers.provider.getSigner(deployer)).burnGem(deployer, gemId, burnAmount);
  ...
  it('addGemsAndCatalysts should fail for existing gemId', async function () {
    const { gemsCatalystsRegistry, powerGem, commonCatalyst, accounts } = await setupGemsAndCatalysts();
    const { deployer } = accounts;
    expect(gemsCatalystsRegistry.
      connect(ethers.provider.getSigner(deployer)).
  ...
  it('addGemsAndCatalysts should add catalystExample', async function () {
    const { gemsCatalystsRegistry, catalystExample, accounts } = await setupGemsAndCatalysts();
    const { deployer } = accounts;
    const tx = await gemsCatalystsRegistry.
      connect(ethers.provider.getSigner(deployer)).
    ...
    expect(await gemsCatalystsRegistry.isCatalystExists(catalystId)).to.equal(true);
  });

  it('addGemsAndCatalysts should fail for gem id not in order', async function () {
    const { gemsCatalystsRegistry, gemNotInOrder, accounts } = await setupGemsAndCatalysts();
    const { deployer } = accounts;
    expect(gemsCatalystsRegistry.
      connect(ethers.provider.getSigner(deployer)).
