...
describe("ERC677Token", function () {
  async function initContracts(name: string, symbol: string) {
    ...
    const token: Contract = await initContract("ERC20Token", accounts.deployer, [name, symbol, accounts.deployer]);
    const tokenReceiver: Contract = await initContract("MockERC677Receiver", accounts.deployer, []);
  ...
  async function initContract(contractName: string, deployer: string, params: Array<string>): Promise<Contract> {
    const ethersFactory: ContractFactory = await ethers.getContractFactory(contractName, ethers.provider.getSigner(deployer));
  ...
  it("Transfering tokens to a non receiver contract should fail", async function () {
    const accounts = await getNamedAccounts();
    const { token } = await initContracts("MOCK", "MOCK");
    const emptyContract = await initContract("EmptyContract", accounts.deployer, []);
  ...
  it("Transfering tokens to a contract with fallback function should succeed", async function () {
    const accounts = await getNamedAccounts();
    const { token } = await initContracts("MOCK", "MOCK");
    const fallbackContract = await initContract("FallBackContract", accounts.deployer, []);
