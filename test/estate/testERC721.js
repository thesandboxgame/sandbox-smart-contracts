...
const erc721Tests = require('../erc721')(async () => {
  ...
  await landContract.connect(landContract.provider.getSigner(landAdmin)).functions.setMinter(deployer, true).then(tx => tx.wait());

  let counter = 0;
  async function mint(to) {
    const landTx = await landContract.connect(landContract.provider.getSigner(deployer)).functions.mintQuad(to, 1, counter, counter, '0x');
    await landTx.wait();
    const tx = await contract.connect(contract.provider.getSigner(to)).functions.createFromQuad(to, to, 1, counter, counter);
