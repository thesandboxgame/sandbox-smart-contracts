...
describe('Estate:CreationAndDestruction', function () {
  ...
  it('creating Estate with many Lands and destroying get them back', async function () {
    ...
    const receipt = await estateContract.connect(estateContract.provider.getSigner(user0)).functions.burnAndTransferFrom(user0, 1, user0).then((tx) => tx.wait());
  ...
  it('creating Estate with many Lands and destroying in 2 step get them back', async function () {
    ...
    const receipt = await estateContract.connect(estateContract.provider.getSigner(user0)).functions.transferFromDestroyedEstate(user0, 1, 0, user0).then((tx) => tx.wait());
    // console.log({gasUsed: receipt.gasUsed.toNumber()});
    helper.checkLandOwnership(selection, user0);
  });
