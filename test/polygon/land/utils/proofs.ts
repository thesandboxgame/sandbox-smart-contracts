...
import {rlp, keccak256, toBuffer, BN} from 'ethereumjs-util';
...
export function getTxBytes(tx) {
  // const common = Common.custom({chainId: 15001, name: 'bor'});
  ...
  return new Promise((resolve, reject) => {
    txTrie.findPath(
      ...
      (err, rawTxNode, reminder, stack) => {
        ...
        const blockHash = toBuffer(tx.blockHash);
        const parentNodes = stack.map((s) => s.raw);
        const root = getRawHeader(block).transactionsTrie;
        console.log('root', root);
        const path = rlp.encode(tx.transactionIndex);
        const value = rlp.decode(rawTxNode.value);
...
export function getReceiptBytes(receipt) {
  return rlp.encode([
    ...
    toBuffer(new BN(receipt.cumulativeGasUsed.toString())),
  ...
  return new Promise((resolve, reject) => {
    receiptsTrie.findPath(
      ...
      (err, rawReceiptNode, reminder, stack) => {
        ...
        const blockHash = toBuffer(receipt.blockHash);
        const parentNodes = stack.map((s) => s.raw);
        const root = getRawHeader(block).receiptTrie;
        const path = rlp.encode(receipt.transactionIndex);
        const value = rlp.decode(rawReceiptNode.raw[1]);
