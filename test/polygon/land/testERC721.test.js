...
const erc721Tests = require('../../erc721')(
  withSnapshot(['MockLandWithMint'], async () => {
    const {landTokenAdmin} = await getNamedAccounts();
    const others = await getUnnamedAccounts();

    const contract = await ethers.getContract('MockLandWithMint');

    const landAsAdmin = await contract.connect(
      ethers.provider.getSigner(landTokenAdmin)
    );
