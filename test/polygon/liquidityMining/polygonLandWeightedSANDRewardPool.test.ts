...
describe('PolygonLandWeightedSANDRewardPool', function () {
  ...
  it('Multiplier changes over time, checking contribution & reward', async function () {
    const {
      rewardPoolContract,
      others,
      REWARD_DURATION,
    ...
    await ethers.provider.send('evm_setNextBlockTimestamp', [
      currentTimestamp + REWARD_DURATION / 2,
    ]);
    await mine();
    ...
    let rewards = await rewardPoolContract.rewards(others[0]);
    ...
    expect(userContribution).to.equal(
      contribution(SMALL_STAKE_AMOUNT, BigNumber.from(1))
    );
    ...
    rewards = await rewardPoolContract.rewards(others[0]);

    expect(rewards).to.be.equal(0);

    const rewardBalance = await rewardTokenContract.balanceOf(others[0]);

    const precisionLost = ACTUAL_REWARD_AMOUNT.sub(rewardBalance);

    expect(rewardBalance).not.to.equal(ACTUAL_REWARD_AMOUNT);
    expect(precisionLost).to.be.at.least(1);
    expect(precisionLost).to.be.at.most(2);
  });
});
