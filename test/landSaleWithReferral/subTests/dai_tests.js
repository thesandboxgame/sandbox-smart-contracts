...
function runDaiTests() {
  describe("LandSaleWithReferral:DAI", function () {
    let initialSetUp;
    ...
    describe("--> Tests with real LANDs", function () {
      ...
      it("DAI is enabled", async function () {
        ...
        const isDAIEnabled = await contracts.landSaleWithReferral.isDAIEnabled();
        assert.ok(isDAIEnabled, "DAI should be enabled");
      });

      it("DAI can be disabled", async function () {
        const {LandSaleAdmin} = initialSetUp;
        await LandSaleAdmin.LandSaleWithReferral.functions.setDAIEnabled(false);
        const isDAIEnabled = await LandSaleAdmin.LandSaleWithReferral.functions.isDAIEnabled();
      ...
      it("DAI cannot be enabled if not admin", async function () {
        ...
        await expectRevert(
          userWithDAI.LandSaleWithReferral.functions.setDAIEnabled(true),
      ...
      it("can buy LAND with DAI and referral", async function () {
        ...
        const tx = await userWithDAI.LandSaleWithReferral.functions.buyLandWithDAI(
          userWithDAI.address,
          userWithDAI.address,
          zeroAddress,
          land.x,
      ...
      it("can buy Land with just enough tokens", async function () {
        ...
        await DaiAdmin.Dai.transfer(userWithoutDAI.address, BigNumber.from(sandToUSD(lands[5].price)));

        await userWithoutDAI.LandSaleWithReferral.functions.buyLandWithDAI(
          userWithoutDAI.address,
          userWithoutDAI.address,
          zeroAddress,
          land.x,
      ...
      it("after buying user owns all LAND bought (empty referral)", async function () {
        ...
        for (let x = land.x; x < land.x + land.size; x++) {
          for (let y = land.y; y < land.y + lands.size; y++) {
            ...
            assert.equal(owner, userWithDAI.address);
    ...
    describe("--> Tests with test LANDs for reserved addresses", function () {
      ...
      it("can buy LAND from a reserved Land if matching address (empty referral)", async function () {
        ...
        const land = lands[0];
        const proof = tree.getProof(calculateLandHash(land));
        await secondUserWithDAI.LandSaleWithReferral.functions.buyLandWithDAI(
