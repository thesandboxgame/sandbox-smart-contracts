...
export const backendAuthWallet = new ethers.Wallet("0x4242424242424242424242424242424242424242424242424242424242424242")
export const zeroAddress = "0x0000000000000000000000000000000000000000";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const signAuthMessageAs = async (wallet: Wallet | SignerWithAddress, ...args: any[]): Promise<string> => {
  const hashedData = ethers.utils.solidityKeccak256(
    ['address', 'address', 'uint256', 'uint256', 'uint256', 'uint256', 'bytes32', 'uint256[]', 'bytes32[]'],
    args
  );
  return wallet.signMessage(ethers.utils.arrayify(hashedData));
}

export const setupAuthValidator = deployments.createFixture(async function (hre) {
...
export const setupEstateSale = deployments.createFixture(async function (hre) {
  ...
  const authValidatorContract = await ethers.getContract('AuthValidator');
  const estateSaleWithAuthContract = await ethers.getContract('EstateSaleWithAuth_0_0');
  const sandContract = await ethers.getContract('Sand');
  const proofs: SaltedProofSaleLandInfo[] = fs.readJSONSync('./secret/estate-sale/hardhat/.proofs_EstateSaleWithAuth_0_0.json')
