...
module.exports = (init, extensions) => {
  ...
  function preTest(test) {
    return async () => {
      ...
      const nonReceivingFactory = new ContractFactory(nonReceiving.abi, nonReceiving.bytecode, ethersProvider.getSigner(deployer));
  ...
  function testSafeTransfers(it, data) {
    ...
    if (data) {
      safeTransferFrom = (contract, from, to, tokenId) => {
        return contract['safeTransferFrom(address,address,uint256,bytes)'](from, to, tokenId, data).then(tx => tx.wait());
      };
    }

    it(prefix + 'safe transfering one NFT results in one erc721 transfer event', async function ({contractAsOwner, owner, user0, tokenIds}) {
      const receipt = await safeTransferFrom(contractAsOwner, owner, user0, tokenIds[0]);
    ...
    it(prefix + 'safe transfering to zero address should fails', async function ({contractAsOwner, owner, tokenIds}) {
      await expectRevert(safeTransferFrom(contractAsOwner, owner, zeroAddress, tokenIds[0]));
    });

    it(prefix + 'safe transfering one NFT change to correct owner', async function ({contract, contractAsOwner, owner, user0, tokenIds}) {
      await safeTransferFrom(contractAsOwner, owner, user0, tokenIds[0]);
      const newOwner = await contract.callStatic.ownerOf(tokenIds[0]);
      assert.equal(newOwner, user0);
    });

    it(prefix + 'safe transfering from without approval should fails', async function ({contractAsUser0, owner, user0, tokenIds}) {
      await expectRevert(safeTransferFrom(contractAsUser0, owner, user0, tokenIds[0]));
    });

    it(prefix + 'safe transfering to a contract that do not accept erc721 token should fail', async function (
      {deployERC721TokenReceiver, contract, contractAsOwner, owner, tokenIds}
    ) {
      const receiverContract = await deployERC721TokenReceiver(contract.address, false, true);
      const receiverAddress = receiverContract.address;
      await expectRevert(safeTransferFrom(contractAsOwner, owner, receiverAddress, tokenIds[0]));
    });

    it(prefix + 'safe transfering to a contract that do not return the correct onERC721Received bytes shoudl fail', async function (
      {deployERC721TokenReceiver, contract, contractAsOwner, owner, tokenIds}
    ) {
      const receiverContract = await deployERC721TokenReceiver(contract.address, true, false);
      const receiverAddress = receiverContract.address;
      await expectRevert(safeTransferFrom(contractAsOwner, owner, receiverAddress, tokenIds[0]));
    });

    it(prefix + 'safe transfering to a contract that do not implemented onERC721Received should fail', async function (
      {deployNonReceivingContract, contract, contractAsOwner, owner, tokenIds}
    ) {
      const receiverContract = await deployNonReceivingContract(contract.address);
      const receiverAddress = receiverContract.address;
    ...
    it(prefix + 'safe transfering to a contract that return the correct onERC721Received bytes shoudl succeed', async function (
      {deployERC721TokenReceiver, contract, contractAsOwner, owner, tokenIds}
    ) {
      const receiverContract = await deployERC721TokenReceiver(contract.address, true, true);
      const receiverAddress = receiverContract.address;
      await safeTransferFrom(contractAsOwner, owner, receiverAddress, tokenIds[0]);
      const newOwner = await contract.callStatic.ownerOf(tokenIds[0]);
      assert.equal(newOwner, receiverAddress);
    });
  }
