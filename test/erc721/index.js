  ...
  if (extensions.batchTransfer) {
    describe("batchTransfer", function (it) {
      it("batch transfer of same NFT ids should fails", async function ({contractAsOwner, owner, user0, tokenIds}) {
        await expectRevert(
          contractAsOwner
            .batchTransferFrom(owner, user0, [tokenIds[1], tokenIds[1], tokenIds[0]], emptyBytes)
            .then((tx) => tx.wait())
        );
      });
      ...
      describe("mandatory batchTransfer", function (it) {
        it("batch transfering to a contract that do not implements mandatory erc721 receiver but implement classic ERC721 receiver and reject should not fails", async function ({
          ...
          await contractAsOwner
            .batchTransferFrom(owner, receiverAddress, [tokenIds[0]], emptyBytes)
            .then((tx) => tx.wait());
        ...
        it("batch transfering to a contract that implements mandatory erc721 receiver (and signal it properly via 165) should fails if it reject it", async function ({
          ...
          const receiverAddress = receiverContract.address;
          await expectRevert(
            contractAsOwner.batchTransferFrom(owner, receiverAddress, [tokenIds[0]], emptyBytes).then((tx) => tx.wait())
          );
        });
  ...
  if (extensions.batchTransfer) {
    describe("safe batch transfer", function (it) {
      it("safe batch transfer of same NFT ids should fails", async function ({
        ...
        await expectRevert(
          contractAsOwner
            .safeBatchTransferFrom(owner, user0, [tokenIds[0], tokenIds[1], tokenIds[0]], emptyBytes)
            .then((tx) => tx.wait())
        );
      });
  ...
  function testSafeTransfers(it, data) {
    ...
    if (data) {
      safeTransferFrom = (contract, from, to, tokenId) => {
        return contract["safeTransferFrom(address,address,uint256,bytes)"](from, to, tokenId, data).then((tx) =>
          tx.wait()
        );
      };
    }

    it(prefix + "safe transfering one NFT results in one erc721 transfer event", async function ({
      contractAsOwner,
      owner,
      user0,
      tokenIds,
    ...
    it(prefix + "safe transfering one NFT change to correct owner", async function ({
      contract,
      contractAsOwner,
      owner,
      user0,
    ...
    it(prefix + "safe transfering from without approval should fails", async function ({
      contractAsUser0,
      owner,
      user0,
      tokenIds,
    ...
    it(
      ...
      async function ({deployERC721TokenReceiver, contract, contractAsOwner, owner, tokenIds}) {
