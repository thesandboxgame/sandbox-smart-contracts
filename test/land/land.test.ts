...
import {setupUsers, setupUser, waitFor} from '../utils';
...
const setupTest = deployments.createFixture(
  async (): Promise<{
    ...
    const landOwners = await setupUsers(unnamedAccounts, {Land});
    const landAdmin = await setupUser(namedAccounts.landAdmin, {Land});
    return {Land, landOwners, landAdmin};
  }
);

describe('Land test', function () {
  describe('Mint and transfer full quad', function () {
    it('testing transferQuad', async function () {
      const {landAdmin, landOwners} = await setupTest();
      const bytes = '0x3333';

      await waitFor(landAdmin.Land.setMinter(landOwners[0].address, true));

      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 12, 0, 0, bytes)
      );

      const num = await landOwners[0].Land.balanceOf(landOwners[0].address);
      expect(num).to.equal(144);
      await waitFor(
        landOwners[0].Land.transferQuad(
      ...
      const num1 = await landOwners[0].Land.balanceOf(landOwners[0].address);
      expect(num1).to.equal(0);
      const num2 = await landOwners[0].Land.balanceOf(landOwners[1].address);
  ...
  describe('Mint and transfer a smaller quad', function () {
    it('transfering a 3X3 quad from a 12x12', async function () {
      ...
      await waitFor(landAdmin.Land.setMinter(landOwners[0].address, true));
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 12, 12, 12, bytes)
      );
      const num = await landOwners[0].Land.balanceOf(landOwners[0].address);
      expect(num).to.equal(144);
  ...
  describe('Mint 1x1 and transfer it', function () {
    it('Mint and transfer 1x1', async function () {
      ...
      await waitFor(landAdmin.Land.setMinter(landOwners[0].address, true));
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 1, 1, 1, bytes)
      );
      const num = await landOwners[0].Land.balanceOf(landOwners[0].address);
      expect(num).to.equal(1);

      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 1, 2, 1, bytes)
      );
      ...
      const num2 = await landOwners[0].Land.balanceOf(landOwners[0].address);

      expect(num2).to.equal(1);

      const num3 = await landOwners[0].Land.balanceOf(landOwners[1].address);
  ...
  describe('Mint and transfer all its smaller quads', function () {
    it('transfering a 1X1 quad from a 3x3', async function () {
      ...
      await waitFor(landAdmin.Land.setMinter(landOwners[0].address, true));
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 3, 3, 3, bytes)
      );
      const num = await landOwners[0].Land.balanceOf(landOwners[0].address);
  ...
  describe('transfer batch', function () {
    it('testing batchTransferQuad', async function () {
      ...
      await waitFor(landAdmin.Land.setMinter(landOwners[0].address, true));
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 24, 0, 0, bytes)
      );
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 12, 300, 300, bytes)
      );
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 6, 30, 30, bytes)
      );
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 3, 24, 24, bytes)
      );
      await waitFor(
        landOwners[0].Land.batchTransferQuad(
          landOwners[0].address,
          landOwners[1].address,
          [24, 12, 6, 3],
          [0, 300, 30, 24],
  ...
  describe('GasTest for single transfer', function () {
    ...
    it('testing transfering 50 lands', async function () {
      ...
      for (let i = 1; i < 6; i++) {
        await waitFor(
          landOwners[0].Land.mintQuad(landOwners[0].address, 1, i, i, bytes)
        );
      }

      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 3, 18, 18, bytes)
      );

      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 6, 54, 54, bytes)
      );
  ...
  describe('GasTest for tranfer quad', function () {
    ...
    it('batch transfer 2 576 land quads', async function () {
      ...
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 24, 48, 48, bytes)
      );
    ...
    it('batch transfer 3 576 land quads', async function () {
      ...
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 24, 72, 72, bytes)
      );
    ...
    it('batch transfer 4 576 land quads', async function () {
      ...
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 24, 96, 96, bytes)
      );
    ...
    it('batch transfer 5 576 land quads', async function () {
      ...
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 24, 120, 120, bytes)
      );
    ...
    it('batch transfer FROM 5 576', async function () {
      ...
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 24, 24, 24, bytes)
      );
      ...
      const receipt = await waitFor(
        landOwners[0].Land.batchTransferFrom(
          landOwners[0].address,
          landOwners[1].address,
          array,
          bytes
  ...
  describe('Gas consumption for 1x1 from each different size of quad', function () {
    it('transfer quad 1X1', async function () {
      ...
      await waitFor(landAdmin.Land.setMinter(landOwners[0].address, true));
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 1, 0, 0, bytes)
      );
    ...
    it('transfer 1x1 quad from 3X3', async function () {
      ...
      await waitFor(landAdmin.Land.setMinter(landOwners[0].address, true));
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 3, 0, 0, bytes)
      );
    ...
    it('transfer 1x1 quad from 6X6', async function () {
      ...
      await waitFor(landAdmin.Land.setMinter(landOwners[0].address, true));
      await waitFor(
        landOwners[0].Land.mintQuad(landOwners[0].address, 6, 0, 0, bytes)
      );
  ...
  it('transferFrom quad 12X12', async function () {
    ...
    //1, 1, 1
    const receipt = await waitFor(
      landOwners[0].Land.transferFrom(
        landOwners[0].address,
        landOwners[1].address,
        818
      )
