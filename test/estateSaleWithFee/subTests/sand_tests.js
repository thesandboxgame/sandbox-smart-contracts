...
function runSandTests() {
  describe("EstateSaleWithFee:SAND", function () {
    let initialSetUp;
    ...
    describe("--> Tests with real LANDs", function () {
      ...
      it("can buy Land with just enough tokens", async function () {
        ...
        await SandAdmin.Sand.transfer(userWithoutSAND.address, BigNumber.from(land.price).div(100).mul(105));
      ...
      it("CANNOT buy Land without adding on a 5% fee", async function () {
        const {userWithoutSAND, tree, lands, SandAdmin} = initialSetUp;
        const land = lands.find((l) => l.size === 6);
        const proof = tree.getProof(calculateLandHash(land));

        await SandAdmin.Sand.transfer(userWithoutSAND.address, BigNumber.from(land.price).div(100).mul(104));
      ...
      it("CANNOT buy Land without just enough tokens inclusive of fee", async function () {
        const {userWithoutSAND, tree, lands, SandAdmin} = initialSetUp;
        const land = lands.find((l) => l.size === 6);
        const proof = tree.getProof(calculateLandHash(land));

        await SandAdmin.Sand.transfer(
          userWithoutSAND.address,
          BigNumber.from(land.price).div(100).mul(105).sub(BigNumber.from(1))
        );
