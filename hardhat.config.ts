...
import {addSourceFiles} from './utils/hardhatConfig';

// A hack to set the FQN for the contracts to latest
extendEnvironment((hre) => {
  const originalDeploy = hre.deployments.deploy;
  const latest = 'sandbox-smart-contracts-latest/src';
  hre.deployments = {
    ...hre.deployments,
    deploy: async (name: string, options: DeployOptions): Promise<DeployResult> => {
      const fqns = await hre.artifacts.getAllFullyQualifiedNames();
      const contractName = options.contract && typeof (options.contract) === 'string' ? options.contract : name;
      if (!fqns.includes(contractName)) {
        const names = fqns.filter(x => x.endsWith(':' + contractName) && x.startsWith(latest));
        if (names.length === 0) {
          throw new Error('contract not found ' + contractName);
        } else if (names.length !== 1) {
          throw new Error('too many names ' + names.join(','));
        }
        options.contract = names[0];
      }
      return originalDeploy(name, options);
    }
  };
});
