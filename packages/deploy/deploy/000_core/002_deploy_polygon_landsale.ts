...
const func: DeployFunction = async function (hre) {
  ...
  const landContract = await deployments.get('PolygonLandV2'); // L2 Land
  ...
  async function deployLandSale(name: string, landSale: LandSale) {
    ...
    if (!landSaleDeployment) {
      landSaleDeployment = await deploy(deployName, {
        from: deployer,
        linkedData: lands,
        contract: '@sandbox-smart-contracts/core/src/solc_0.6/EstateSale/EstateSaleWithAuth',
        args: [
...
func.dependencies = [
  'PolygonSand_deploy',
  'PolygonLandV2_deploy', // L2 Land
  'Asset_deploy', // L2 Asset
