...
import {
  ...
  zeroAddress,
  assert,
} from './CareBears.fixtures';

// eslint-disable-next-line mocha/no-skipped-tests
describe('RaffleCareBears', function () {
  it('should be able to mint with valid signature', async function () {
    ...
    await setupWave(raffleCareBearsContract, 0, 20, 5, '10', zeroAddress, 0);
    await mint(
      raffleSignWallet,
      deployer,
      0,
      raffleCareBearsContract.address,
      hre.network.config.chainId || 31337,
      '10',
      1
    );

    const transferEvents = await raffleCareBearsContract.queryFilter(
  ...
  it('should be able to personalize with valid signature', async function () {
    ...
    const personalizeEvents = await raffleCareBearsContract.queryFilter(
      raffleCareBearsContract.filters.Personalized()
    );
    ...
    const personalizationOf = await raffleCareBearsContract.personalizationOf(
      tokenId
    );
  ...
  it('should be able to differentiate a personalized asset', async function () {
    ...
    const transferEvents1 = await raffleCareBearsContract.queryFilter(
      raffleCareBearsContract.filters.Transfer(),
    ...
    const transferEvents2 = await raffleCareBearsContract.queryFilter(
      raffleCareBearsContract.filters.Transfer(),
    ...
    const allPersonalizeEvents = await raffleCareBearsContract.queryFilter(
      raffleCareBearsContract.filters.Personalized()
    );

    const personalizeEvents2 = await raffleCareBearsContract.queryFilter(
      raffleCareBearsContract.filters.Personalized(),
  ...
  it('should not be able to personalize twice with the same signature', async function () {
    ...
    const contract = raffleCareBearsContract.connect(
