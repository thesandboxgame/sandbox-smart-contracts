 ...
 */
abstract contract LandBaseTokenV3 is ERC721BaseTokenV2 {
    ...
    function mintAndTransferQuad(address to, uint256 size, uint256 x, uint256 y, bytes calldata data) external {
        ...
        if (exists(size, x, y) == true) {
            ...
            _moveNumNFTPerAddress(msg.sender, to, size * size);
    ...
    function transferQuad(address from, address to, uint256 size, uint256 x, uint256 y, bytes calldata data) external {
        ...
        if (msg.sender != from && !metaTx) {
            require(
                _isOperatorsForAll(from, msg.sender) || _isSuperOperator(msg.sender),
                "not authorized to transferQuad"
            );
        }
        _transferQuad(from, to, size, x, y);
        _moveNumNFTPerAddress(from, to, size * size);
    ...
    function batchTransferQuad(
        ...
        require(from != address(0), "from is zero address");
        require(to != address(0), "can't send to zero address");
        require(sizes.length == xs.length, "sizes's and x's length different");
        require(xs.length == ys.length, "x's and y's length different");
        bool metaTx = msg.sender != from && _isMetaTransactionProcessor(msg.sender);
        if (msg.sender != from && !metaTx) {
            require(_isOperatorsForAll(from, msg.sender) || _isSuperOperator(msg.sender), "not authorized");
        }
        ...
        _moveNumNFTPerAddress(from, to, numTokensTransfered);
    ...
    function setMinter(address minter, bool enabled) external onlyAdmin {
        require(minter != address(0), "address 0 not allowed");
        require(enabled != $minters()[minter], "the status should be different");
    ...
    function _transferQuad(address from, address to, uint256 size, uint256 x, uint256 y) internal {
        ...
        if (size == 1) {
            ...
            _setOwnerData(id1x1, uint160(to));
        } else {
