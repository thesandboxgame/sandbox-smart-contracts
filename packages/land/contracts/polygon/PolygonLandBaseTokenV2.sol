 ...
 */
abstract contract PolygonLandBaseTokenV2 is Initializable, ERC721BaseTokenV2 {
    ...
    function batchTransferQuad(
        ...
        if (_msgSender() != from) {
            require(_isOperatorsForAll(from, _msgSender()) || _isSuperOperator(_msgSender()), "not authorized");
        }
    ...
    function transferQuad(address from, address to, uint256 size, uint256 x, uint256 y, bytes calldata data) external {
        ...
        if (_msgSender() != from) {
            require(
                _isOperatorsForAll(from, _msgSender()) || _isSuperOperator(_msgSender()),
