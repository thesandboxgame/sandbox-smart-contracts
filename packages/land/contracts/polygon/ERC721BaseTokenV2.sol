...
abstract contract ERC721BaseTokenV2 is ERC721BaseToken {
    ...
    function approve(address operator, uint256 id) public virtual override {
        ...
        require(
            owner == msgSender || _isOperatorsForAll(owner, msgSender) || _isSuperOperator(msgSender),
    ...
    function approveFor(address sender, address operator, uint256 id) public virtual {
        ...
        require(owner != address(0), "NONEXISTENT_TOKEN");
        require(
            msgSender == sender || _isOperatorsForAll(sender, msgSender) || _isSuperOperator(msgSender),
            "UNAUTHORIZED_APPROVAL"
        );
    ...
    function burnFrom(address from, uint256 id) external virtual {
        ...
        require(
            msgSender == from ||
                ...
                _isSuperOperator(msgSender) ||
                _isOperatorsForAll(from, msgSender),
            "UNAUTHORIZED_BURN"
        );
        _burn(from, owner, id);
    }
    ...
    function _updateOwnerData(uint256 id, address newOwner) internal {
        uint256 oldData = _getOwnerData(id);
    ...
    function _approveFor(uint256 ownerData, address operator, uint256 id) internal {
        ...
        if (operator == address(0)) {
            _setOwnerData(id, ((ownerData & NOT_ADDRESS) & NOT_OPERATOR_FLAG) | uint256(uint160(owner)));
        } else {
            _setOwnerData(id, ((ownerData & NOT_ADDRESS) | OPERATOR_FLAG | uint256(uint160(owner))));
    ...
    function _batchTransferFrom(address from, address to, uint256[] memory ids, bytes memory data, bool safe) internal {
        mapping(uint256 => address) storage _operators = $operators();
        address msgSender = _msgSender();
        bool authorized = msgSender == from || _isOperatorsForAll(from, msgSender) || _isSuperOperator(msgSender);
    ...
    function _burn(address from, address owner, uint256 id) internal {
        ...
        uint256 storageId = id;
        _setOwnerData(storageId, (_getOwnerData(storageId) & NOT_OPERATOR_FLAG) | BURNED_FLAG);
