...
export async function setupRaffleContract(
  ...
  const RaffleImplementationContract = await ethers.getContractFactory(
    contractName
  );
  ...
  const collectionContractAsOwner = raffleContract.connect(
    deployer
  ) as unknown as GenericRaffle;

  assert.equal(
    (await collectionContractAsOwner.maxSupply()).toString(),
    collectionMaxSupply.toString(),
    'possible collection miss-match'
  );
  ...
  return {
    ...
    randomWallet,
    collectionContractAsRandomWallet: raffleContract.connect(
      randomWallet
    ) as unknown as GenericRaffle,
  };
}
