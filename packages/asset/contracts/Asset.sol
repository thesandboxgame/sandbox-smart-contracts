  ...
  OperatorFiltererUpgradeable
{
  ...
  function initialize(
    ...
    __OperatorFilterer_init(commonSubscription, false, operatorFilterRegistry);
  ...
  function bridgeMint(
    ...
    if (bridgedTokensNonces[originalTokenId] == 0) {
      // increment nonce
      unchecked { creatorNonces[originalCreator]++; }
      // get current creator nonce
  ...
  function recycleBurn(
  ...
  ) external onlyRole(MINTER_ROLE) returns (uint256 amountOfCatalystExtracted) {
    ...
    require(recyclingAmount > 0, "Catalyst tier is not eligible for recycling");
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      require(extractedTier == catalystTier, "Catalyst id does not match");
    ...
    uint256 catalystsExtractedCount =
      totalAmount / recyclingAmounts[catalystTier];
  ...
  function setRecyclingAmount(uint256 catalystTokenId, uint256 amount)
    external
  ...
  function setTokenUri(uint256 tokenId, string memory metadata)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
  ...
  function getRecyclingAmount(uint256 catalystTokenId)
    public
    view
    returns (uint256)
  {
