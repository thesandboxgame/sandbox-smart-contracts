...
{
    // Layer masks
    uint256 CHAIN_MASK = 0xFF;
    uint256 TIER_MASK = 0xFF;
    uint256 REVEALED_MASK = 0x1;
    uint256 NONCE_MASK = 0x3FF;
    uint256 HASH_MASK = 0xFFFFFFFFF;

    // Bit shifts
    uint256 CREATOR_SHIFT = 0;
    uint256 CHAIN_SHIFT = 160;
    uint256 TIER_SHIFT = 168;
    uint256 REVEALED_SHIFT = 176;
    uint256 NONCE_SHIFT = 177;
    uint256 HASH_SHIFT = 194;
    ...
    /// @param revealHash The ipfs hash of the abilities and enhancements of the asset
    ...
    function extractCreatorNonceFromId(
        ...
        creatorNonce = uint16((tokenId >> NONCE_SHIFT) & NONCE_MASK);
        return creatorNonce;
    }
    ...
    function extractRevealHashFromId(
        ...
        revealHash = uint40((tokenId >> HASH_SHIFT) & HASH_MASK);
        return revealHash;
    }
    ...
    function getDataFromTokenId(
        ...
        data.creator = extractCreatorFromId(tokenId);
        data.tier = extractTierFromId(tokenId);
        data.revealed = extractIsRevealedFromId(tokenId);
        data.creatorNonce = extractCreatorNonceFromId(tokenId);
    }
