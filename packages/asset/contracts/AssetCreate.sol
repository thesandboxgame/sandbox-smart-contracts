    ...
    PausableUpgradeable
{
    ...
    /// @param amount The amount of the asset to mint
    /// @param metadataHash The metadata hash of the asset to mint
    /// @param maxSupply The max supply of the asset to mint
    /// @param creator The address of the creator
    function lazyCreateAsset(
        ...
        require(
            authValidator.verify(
                signature,
                _hashLazyMint(creator, signatureNonces[_msgSender()]++, tier, amount, metadataHash, maxSupply)
            ),
            "AssetCreate: Invalid signature"
        );
    ...
    /// @param amounts The amounts of the assets to mint
    /// @param metadataHashes The metadata hashes of the assets to mint
    /// @param maxSupplies The max supplies of the assets to mint
    ...
    function lazyCreateMultipleAssets(
        ...
        for (uint256 i = 0; i < tiers.length; i++) {
            ...
            if (tokenIds[i] == 0) {
                ...
                require(availableToMint[tokenIds[i]] >= amounts[i], "AssetCreate: Max supply reached");
                availableToMint[tokenIds[i]] -= amounts[i];
            }
        }
        ...
        emit AssetBatchLazyMinted(_msgSender(), creators, tokenIds, tiers, amounts, metadataHashes);
    }
    ...
    /// @notice Set a new vault address, limited to DEFAULT_ADMIN_ROLE only
    /// @dev Change the address of the vault that distributes the catalyst value to the creators
