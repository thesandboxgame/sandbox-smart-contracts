...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const currentMigrationContract = await read(
    'AssetAttributesRegistry',
    'migrationContract'
  );
  const MigrationContract = await deployments.get(
    'CollectionCatalystMigrations'
  );

  if (
    currentMigrationContract.toLowerCase() !==
    MigrationContract.address.toLowerCase()
  ) {
    await execute(
      'AssetAttributesRegistry',
