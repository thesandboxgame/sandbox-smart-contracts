...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  try {
    assetData = JSON.parse(
      fs
        .readFileSync(
          `data/giveaways/asset_giveaway_4/assets_${hre.network.name}.json`
        )
        .toString()
    );
  } catch (e) {
  ...
  if (assetData.length === 0) {
    log('no assets for Asset_Giveaway_4');
    return;
  }
  ...
  if (network.name !== 'hardhat') {
    fs.writeFileSync(
      `./secret/.asset_giveaway_4_claims_proofs_${chainId}.json`,
