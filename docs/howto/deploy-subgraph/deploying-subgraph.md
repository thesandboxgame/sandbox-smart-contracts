...
For an NFT or gaming application, we can find directly the owner of a NFT or an item based on their ID.  
For doing this, we only have to request a read operation on the contract.  
But more advanced queries like finding all NFT or items associated to a user is not possible only by querying the read method's on contract.  
For doing this we would have to process every transfer event emmitted, agregate data and process.
This is not possible for a dapp only running on a browser.  
With a server thinks are not much better, because this require a centralized cache to store data and thus it introduce a security weakness.  
That is why The Graph network was created.

The graph is an opensource software based on a decentralized protocol that work as a cache  
...
Were as a classic database, on blockchains you cannot perform a query on data by criteria.
...
They needs to attach GRT to these subgraphs.    
...
### 3.1 setting subgraph name

You will be asked to provide a subgraph name.
The require format is 
...
You will be ask to choose a contract name for ts object that will bind your ethereum contract.  
...
## 3 Subgraph manifest file  

Open the subgraph.yaml file at the root of your project.  
You should know see something like this.  
...
The first part of the datasource define informations about the type of blockchain, its network and the contract from which the subgraph was generated.  
...
The other part of the file tells us that our subgraph will behave like a handler processing every type of event from our contract by calling
callback methods on it.  
Here are the ts event associated to solidity contract event.
...
The Graph generate you a mapping.ts file in src folder that contains empty handler to process event
...
## 6 Generate typescript binded entities 
...
This will generate typescript binded entities to previously defined entites in the file schema.ts  
...
```
## 7 Write events handlers

Once we have our binded entites in typescript, we can use it write our handler that will process events, create entities and save them.  
...
This will generate artifact it build sub folder.  
You will find a Faucet.wasm file and a Faucet.json file.  

### 8.2 Subgraph creation on dashbord
...
For Authentification and then deployment, you will need an Access Token. You can find it on your dashbord.  
...
```
## 9 Use your subgraph for querying objects
...
```
This feature is usefull when you want to retrieve your data into buckets.
...
for example the first 5 event with timestamp greatter or equal than 1641215535  
