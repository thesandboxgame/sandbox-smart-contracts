...
async function getValues({assetId, originalAssetId, fromBlockHash, catalystRegistry}) {
  ...
  const catalystAppliedEvents = await catalystRegistry.queryFilter(
    catalystRegistry.filters.CatalystApplied(), // TODO originalAssetId ?
    fromBlockHash
  );
  const lastCatalystEvent = catalystAppliedEvents[catalystAppliedEvents.length - 1];
  // console.log({lastCatalystEvent: JSON.stringify(lastCatalystEvent), assetId, originalAssetId, fromBlockHash});
  ...
  // console.log({catalystId, seed, gemIds, blockHashes});
